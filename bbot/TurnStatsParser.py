...
class TurnStatsParser(StatsParser):
    ...
    def on_match(self,app,line,which):
        ...
        elif which == 260: realm.population.pop_support = self.get_num()
        elif which == 270: realm.food.units = self.get_num()
        elif which == 280: army.agents.number = self.get_num()
        elif which == 290: army.headquarters.number = self.get_num()
        elif which == 300: army.sdi = self.get_num()
        elif which == 500: realm.population.taxearnings = self.get_num()
        elif which == 510: regions.mountain.earnings = self.get_num()
        elif which == 520: regions.coastal.earnings = self.get_num()
        elif which == 530: regions.desert.earnings = self.get_num()
        elif which == 540: regions.river.earnings = self.get_num()
        elif which == 550: regions.agricultural.foodyield = self.get_num()
        elif which == 310: realm.turns.years_freedom = self.get_num()
        elif which == 320: realm.turns.years_protection = self.get_num()
