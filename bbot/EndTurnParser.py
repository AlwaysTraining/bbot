...
class EndTurnParser(StatsParser):
    ...
    def on_match(self,app,line,which):
        ...
        if which == 1610: app.data.realm.population.growth = self.get_num(0)
        elif which == 1620: app.data.realm.population.growth = -self.get_num(0)
        elif which == 1630: app.data.realm.food.spoilage = self.get_num(0)
        elif which == 1640: app.data.realm.food.randomly_eaten = self.get_num(0)
